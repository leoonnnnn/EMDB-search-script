...
do
  case $o in
    (f) listfile=$OPTARG;;     #automate this too, or let user search from command line, mb rename the searches for easier debugging
    (o) outdir=$OPTARG;;       #download location has to exist, change this so if folder doesnt exist create it
    (c) cif=true;;
    (p) pdb=true;;
    (a) pdb1=true;;
    (x) xml=true;;
...
IFS=',' read -ra tokens <<< "$contents"        #use this shit to enable tags/keywords when searching, actually prob a better way to do it
